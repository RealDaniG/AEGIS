...
class AlertManager:
    ...
    def __init__(self):
        ...
        self.thresholds: Dict[MetricType, Dict[str, Any]] = {}
        self.notification_callbacks: List[Callable] = []
        self._setup_default_thresholds()
    
    def _setup_default_thresholds(self):
        """Configura umbrales de alerta por defecto"""
        self.thresholds = {
    ...
    def evaluate_metric(self, metric: Metric):
        """Evalúa una métrica contra umbrales para generar alertas"""
        try:
            # Obtener configuración de umbrales
            config = self.thresholds.get(metric.metric_type, {})
            warning_threshold = config.get("warning_threshold", float('inf'))
            critical_threshold = config.get("critical_threshold", float('inf'))
            operator = config.get("operator", "greater_than")
            
            alert_level = None
            threshold_value = 0.0  # Initialize with default value
...
class NodeManager:
    ...
    def update_node_status(self, node_id: str, status: NodeStatus, health_score: Optional[float] = None):
...
class DashboardServer:
    ...
    def _setup_socketio_events(self):
        ...
        def handle_connect():
            # Use session ID instead of request.sid
    ...
    def _setup_alert_notifications(self):
        ...
        def notify_alert(alert: Alert):
            # Use socketio.emit without room parameter for compatibility
            ...
            while True:
                try:
                    ...
                    for metric in new_metrics:
                        ...
                        self.socketio.emit('new_metric', asdict(metric))
            ...
            <div class="bg-white rounded-lg shadow-md p-6 metric-card">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-red-500">
                        <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 10-2 0v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                        </svg>
                    </div>
                </div>
            </div>
...
def start_dashboard(config: dict):
    ...
    """
    try:
        host = config.get("host", "0.0.0.0")  # Default to all interfaces
        port = int(config.get("dashboard_port", 8090))  # Use 8090 as default to match config
